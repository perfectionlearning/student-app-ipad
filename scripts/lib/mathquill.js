...
var manageTextarea = (function() {
  ...
  return function manageTextarea(el, opts) {
    ...
    function select(text) {
	  ...
	  // DG: Added try block because this was crashing in IE11
	  try {
...
function createRoot(jQ, root, textbox, editable) {
  ...
  textarea.focus(function(e) {
    ...
    //@jd prevents default keyboard on iPad
    e.preventDefault();
  }).blur(function(e) {
...
//  bind(VanillaSymbol,'\\wedge ','&and;');
  bind(BinaryOperator,'\\wedge ',' and ');	// DG: Changed to text. Also changed to a binary operator to modify fraction handling.

//LatexCmds.or = LatexCmds.lor = LatexCmds.vee = bind(VanillaSymbol,'\\vee ','&or;');
LatexCmds.or = LatexCmds.lor = LatexCmds.vee = bind(BinaryOperator,'\\vee ',' or ');	// DG: Changed to text. Also changed to a binary operator to modify fraction handling.
...
//  bind(BinaryOperator,'\\varnothing ','&empty;');
  bind(BinaryOperator,'\\varnothing ','no solutions');	// DG: Changed to text.
          ...
          if (prop === false || prop instanceof MathBlock) {
            ...
            if (prop instanceof MathBlock) {
              ...
              if (cached) {
				...
				// DG: Fixed a bug. offset(self) can be undefined.
				var ofsSelf = offset(self);
				if (ofsSelf !== undefined)
				{
					var pageX = ofsSelf.left;
    ...
    if (arguments.length > 1)
      return this.each(function() {
        ...
        if (cursor) {
		  ...
		  //-----------------------------------------
		  // DG: Crazy hack to allow cursor movements
		  //-----------------------------------------
		  if (latex === '!RIGHT')
