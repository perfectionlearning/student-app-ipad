...
$(function() {
	...
	var startRequirements = {
		...
		bookDefLoaded: false,		// BAD error checking (fatal error)
		activityManifest: false,	// Poor error checking (message, no way to recover)
		standardsList: false,		// Poor error checking (message, no way to recover)
	...
	function startApp(flag)
	{
		...
		if (done)
		{
			...
			if (name && app.objectList[name])
			{
				...
				app.linkToObject('TOC');	// Default context
		}
	}
	...
	//
	//=======================================================
	function setCurNav(name)
	{
		var nav;
		for (var i = 0; i < app.schemeList.length; i++)
		{
			nav = app.findObject(name, app.schemeList[i]);
			if (nav.length)
				return app.setNav(nav[0], nav[1], nav[2], nav[3], nav[4]);
		}
	}
...
//		interval = setInterval(gameLoop, desiredRate);	// Was running faster than the desired rate, but that makes the interval quite low
	...
	{
		app.loadBookDefinition(bookDefLoaded);	// Load and process the book definition file
		...
		// Fetch the lesson plan / Standards code correlations
		$.getJSON(app.bookPath + app.objStandardsFile).done(standardsLoaded).fail(startupError);

		// Fetch the activities manifest file
		$.getJSON(app.bookPath + app.paths.jsact + "manifest.json").done(manifestLoaded).fail(startupError);
	}
...
// Can receive action as second parameter.
// Default action in app.loadFailed is app.resetView
//=======================================================
app.failHandler = function(type) {
	var actions = {
		solution: app.refreshSolution,
		step: app.resetStep,
		submit: app.resetSubmit,
	}
	var action = arguments[1] || actions[type];
	return function(err) {
		if (app.getError(err) === 'NotLoggedIn')
			app.needLogin();
		else
			app.loadFailed(type, action);
	}
}
//=======================================================
// Choose the correct lists and selections to display for
// the top navigation pulldowns.
//=======================================================
function updateNavBar()
{
	var nav = app.curNav;

	// If we're in a "special" location, the nav bar is meaningless
	var ch = nav.chapter;
	if (ch < 0)
		return;

	var scheme = nav.scheme;

	var top = app.getLinearTopic(ch, nav.unit, nav.topic, scheme);

	app.navBarDef.chapter.data = app.getChapterList('', false, scheme);
	app.navBarDef.chapter.selection = ch;

	app.navBarDef.topic.data = app.chapterTopics[scheme][ch];
	app.navBarDef.topic.selection = top;

	app.navBarDef.section.data = app.sectionsInTopic[scheme][ch][top];
	app.navBarDef.section.selection = nav.section;
}
...
app.getVersionString = function()
{
	...
	var out = 'Version ' +
		app.majorVersion + '.' + instVer +
		', build ' + app.minorVersion;

	if (app.server !== 'live' && app.server !== 'livedb')	// @FIXME/dg: Combine
		out += ', ' + app.server;
...
//=======================================================
//=======================================================
app.testResize = function()
{
	var w = 1000;
	var h = 800;

	topContainer.width(w);
	$('#Stage').width(w);
	topContainer.height(h);
	$('#Stage').height(h);
	fw.stageSize([w,h]);
	fw.resetLayout();
}
