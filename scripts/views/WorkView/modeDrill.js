//===============================================================================
// OHW i-Practice/Quizlet/iHomework/drill/forced mastery/punish the weak mode
//
// Plugin required API:
//   getNavOptions
//   formatRoute
...
;(function() {
	...
	//   statusOverlay: Show statusOverlay indicator (mutually exclusive with "score")
	...
	//----------------------------------------------------
	var layoutOptions = {
		statusOverlay: true,	// Show status overlay
	...
	API.isLockedOut = function(assign, probIndex)
	{
		...
		return (status === "Locked" || status === "Correct");
	}
	...
	{
		// Determine whether there are any questions remaining
		var action = 'submitAllCorrect';		// Assume the assignment is perfect

		for (var i = 0; i < app.problemList.length; i++)
		{
			var val = app.problemList.at(i);
			var status = val.get('status');

			if (status === 'New')
				return 'nextQuestion';		// Found an unanswered question.

			if (status === 'Locked')
				action = 'submitMixedResults';	// Found a wrong answer. Mixed, or perhaps not all are done.
		}

		// All questions were answered. Some may be wrong.
		return action;
	}
