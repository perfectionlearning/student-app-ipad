//===============================================================================
// Single problem (review, development) mode plugin
//
// Plugin required API:
//   getNavOptions
//   formatRoute
...
;(function() {
	...
	{
		return "Question " + (app.curProbIndex+1);
	}
	...
	{
		// I don't like the external access (app namespace), but without a
		// nice mediator or other isolation layer, it will have to do for now.
		app.curAssign = null;

		app.problemList.reset([{id:problemID}])
		app.curProbIndex = 0;

		var problem = app.problemList.at(0);

		problem.setMode('direct');
		return problem.fetch().done(app.modes.createState);
	}
	...
	//=======================================================
//	app.modes.register("SingleProblem", API);
	app.modes.register("TEST", API);
